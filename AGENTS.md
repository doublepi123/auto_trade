...
- Long validation must use a persistent job with a recorded PID/job ID, exact command/environment, log, exit status, and immutable source snapshot (commit plus any uncommitted patch and untracked test inputs). A tool timeout or interrupted polling is not a test failure: inspect the original job and resume waiting before rerunning. Never start a duplicate while the original is alive; choose execution limits from measured lane durations.
- Keep one validation ledger per candidate/baseline snapshot. Reuse completed successful lanes only when source, tests, configuration, dependencies, selection and environment are unchanged and the evidence is complete. Diagnose failures before retrying only the affected lanes; preserve the original failure and its explanation. Changed inputs invalidate affected evidence. A green retry alone does not resolve a concurrency failure, and partial coverage never replaces the full coverage gate.
