...
> **Last updated:** 2026-06-23 · **Commit:** `610d674` (worktree-p153-p162-platform-completion) · **Generated by:** `/init-deep` (update mode)
...
│   │   ├── platform/                       # P149–P152+P156+P153–P162：策略插件 SDK + 统一事件流 + 组合交易 + Paper Broker + 组合风控 + 平台回放/归因/版本化/kill-switch
│   │   │   ├── __init__.py
...
│   │   │   ├── runner.py                   # PlatformRunner（backtest/paper/live；多 symbol、PaperBroker + RiskEngine；submit_intent 公开入口）
│   │   │   ├── replay.py                   # EventReplayer 事件回放
│   │   │   ├── simbroker.py                # 简化回测撮合器（保留；runner 已改用 PaperBroker）
│   │   │   ├── paper_broker.py             # P152/P154/P155：真实成交仿真（partial fill/滑点/费用/撤改单/止损/追踪止损/OCO，paper_orders 持久化 + from_db 重载）
...
│   │   │   ├── backtest_service.py         # P153：平台回测（strategy over bars via PaperBroker）
│   │   │   └── api.py                      # /api/platform/*（strategies/backtest/snapshot/events/replay）
...
`pytest.ini` 已挂上 `pytest-cov`，运行 `pytest` 时自动生成 terminal 报告。`.coveragerc` 排除 `app/main.py` / `app/runner.py` / `app/database.py` / `app/config.py` / `app/api/ws.py`（这些是启动/事件循环粘合层，单测覆盖成本高）。当前基线：**1299 passed**（平台层 P149–P152 + P156 + P153–P162 已并入）。提交 PR 时如改动某模块覆盖率下降超过 5%，请在描述里说明。
...
| 平台回测 | `POST /api/platform/backtest`（任一已注册策略在 K 线上跑 PaperBroker，返回 equity/fills/positions；422 缺字段、404 未知策略；带 API key） |
