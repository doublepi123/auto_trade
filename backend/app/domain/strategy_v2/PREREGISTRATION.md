...
结论：9.2 的 KILL 条件**已经成立**。可以在 95% 水平上排除「大到能覆盖成本的 gross edge」。继续为该假设的统计量投入工程，属于**已宣告的徒劳（declared futility）**。
...
- 本节**不引入**任何代码常量或运行期开关：它是人工判定的书面规则，不是自动化的 kill switch。自动晋级永久禁止（第 7 节），自动弃置同样不存在。
