...
class TestStrategyV2ShadowApi:
    ...
    def test_portfolio_routing_report_is_forward_only_and_read_only(
        ...
        fixed_75bps = next(
            item
            for item in body["variants"]
            if item["policy"] == "VWAP_EDGE_75BPS_POOL"
        )
        assert (
            fixed_75bps["edge_filter"]
        ...
        assert (
            risk_group_leave_one_out["edge_filter"]
            == "RISK_GROUP_LOO_OBS_COST_TO_75BPS"
        )
        sector_leave_one_out = next(
            item
            for item in body["variants"]
            if item["policy"]
            == "SECTOR_LOO_OBS_75BPS_POOL"
        ...
        selected_zscore = next(
            item
            for item in body["variants"]
            if item["policy"] == "SELECTED_ZSCORE_OBS_75BPS_POOL"
        )
        assert (
        ...
        weighted_rotation_zscore = next(
            item
            for item in body["variants"]
            if item["policy"] == "ROTATION_IV_WEIGHTED_ZSCORE_POOL"
        )
        assert (
            weighted_rotation_zscore["edge_filter"]
            == "ZSCORE_OBS_COST_TO_75BPS"
        )
        net_edge_rotation_zscore = next(
            item
            for item in body["variants"]
            if item["policy"] == "ROTATION_IV_NET_EDGE_ZSCORE_POOL"
        )
        assert (
