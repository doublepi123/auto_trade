describe('Strategy v2 shadow lab', () => {
  ...
  it('shows shadow evidence and paper execution state', () => {
    ...
    cy.get('[data-testid="opening-momentum-latest"]')
      ...
      .and('contain', '24.0 bps')
      .and('contain', '42.00%')
      .and('contain', '$31.00M')
      .and('contain', '$1.25B')
      .and('contain', '2.48%')
      .and('contain', '-35.0 bps')
      .and('contain', '25.0 bps')
      .and('contain', '105.0 bps')
      .and('contain', '-8.0 bps')
      .and('contain', '-12.0 bps')
      .and('contain', '-10.0 bps')
      .and('contain', '46.0 bps')
    cy.get('[data-testid="opening-momentum-metrics"]')
      .should('contain', '66')
      .and('contain', '93.6 bps')
    cy.get('[data-testid="opening-momentum-variants"] .el-table__row')
    ...
    cy.get('[data-testid="opening-momentum-variants"]')
      ...
      .and('contain', '至 5.0 bps')
      .and('contain', '弱广度 + PANW')
      .and('contain', '弱广度 + 稀疏指数候选组')
      .and('contain', '弱广度 - MRVL')
      .and('contain', 'ETF 状态 + 路径效率')
      .and('contain', 'ETF 状态 + CRWD')
      .and('contain', 'ETF 状态 + TRV')
      .and('contain', '弱广度 + 4% 灾难止损')
      .and('contain', '开盘区间止损')
      .and('contain', '5 分钟 ORB 突破')
      .and('contain', '活跃成交 ORB')
      .and('contain', '全指数目录 5 分钟 ORB')
      .and('contain', '全指数目录活跃 ORB Top5')
      .and('contain', '全指数目录相对开盘量 ORB Top5')
      .and('contain', '相对量 Top5 + 涨幅重排')
      .and('contain', '相对量 Top5 + 涨幅重排 + 突破深度 10bp')
      .and('contain', '开盘活跃度 Top 20 后突破')
      .and('contain', '同窗量 14 日 / 至少 1.00 / Top 5 后突破')
      .and('contain', 'Top 5 后按开盘涨幅重排突破')
      .and('contain', '突破深度门槛 10.0 bps')
      .and('contain', '候选突破深度')
      .and('contain', '12.5 bps')
      ...
      .and('contain', '动量延续')
      .and('contain', '广度过滤')
      .and('contain', '广度 + 末 5 分钟')
      .and('contain', '末 5 分钟过滤')
      .and('contain', '0.0 bps')
      .and('contain', '-25.0 bps')
      .and('contain', '95.00%')
      .and('contain', 'META.US')
    ...
    cy.get('[data-testid="shadow-portfolio-table"]')
      ...
      .and('contain', '点时稳健轮动池 · 权重偏离优先')
      .and('contain', '点时稳健轮动池 · 净 VWAP 优先')
      .and('contain', '固定成本 → 75bp')
      .and('contain', '观测成本 → 75bp')
      .and('contain', '风险组残差 + 观测成本 → 75bp')
      .and('contain', '风险组留一残差 + 观测成本 → 75bp')
      .and('contain', '细行业留一残差 + 观测成本 → 75bp')
      .and('contain', '双周期 z-score + 观测成本 → 75bp')
      .and('contain', '观测成本缺失 8')
      .and('contain', '无因果候选 2')
      .and('contain', '+0.520%')
      .and('contain', '+0.320%')
