...
describe('Decision Timeline intervention evidence', () => {
  ...
  it('explains response-limit truncation separately from scan-cap truncation', () => {
    cy.stubApi()
    cy.intercept('GET', '/api/intervention-evidence*', {
      body: makeResponse(MIXED_ROWS.slice(0, 2), {
        summary: makeSummary({
          total_evidence: 5,
          scanned_evidence: 5,
          paired_count: 2,
    ...
    cy.get('[data-testid="evidence-truncated-alert"]').should('contain', '仅显示前 2 条（符合筛选共 5 条）')
    cy.get('[data-testid="evidence-incomplete-alert"]').should('not.exist')
    cy.get('[data-testid="evidence-returned-tag"]').should('contain', '返回 2 / 5 条')
  })
