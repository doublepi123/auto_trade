...
| `AUTO_TRADE_UNIVERSE_SELECTION_EXPLORATION_MAX_SYMBOLS` | 先补足已入选风险组的残差基准同伴（可使用仅成交额处于实盘门槛 75%-100% 的 peer-only 标的），再分散加入量化评分与 shadow；不会正式入选、切换主标的或下单，`0` 表示关闭 | `14` |
