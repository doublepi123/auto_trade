...
P12 在**不改任何写路径、不改 LLM 决策逻辑**的前提下，新增一个只读「LLM 优化工作台」页面，把上述三类能力暴露给用户，形成「调 prompt → 看实验胜率 → 看当前指标」的闭环。
...
- 选定后并发拉：
  ...
  - `/api/performance/compare` → 变体对比表（胜率 / 总 PnL / 平均 PnL 列）。
    > **交付说明（2026-05-29）：** 实现以表格呈现变体对比；规格初稿设想的「胜率/PnL 柱状图（复用 SVG 图表栈）」按 YAGNI 未做——表格已完整传达对比数据，柱状图留待后续按需补充。
