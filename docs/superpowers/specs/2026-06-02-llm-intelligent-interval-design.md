...
>
> 本文档是"迭代 0" LLM 智能区间调整的**重写版**设计，目标是引入更复杂的三件套（DataAggregator → LLMAdvisorService → IntervalApplicationService）和双重触发机制（定时 + 波动率）。
>
> **决策依据：**
> - 核心 LLM 区间调整能力已通过 P0/P9/P11（数据真实性修复、模块化 Prompt、自适应特征选择）完整交付。
> - 新设计中的触发机制（4 小时定时 + 5% 波动率）与现有 `LLMAdvisorService` 调度重叠；现状已通过 P22（`02ff712`）补全波动率触发，无需重新搭建三件套。
> - 详情见 [2026-05-31-next-iteration-roadmap.md](../plans/2026-05-31-next-iteration-roadmap.md) 的"显式暂不做"段，以及 [主 Roadmap.md](../../Roadmap.md) 的"下一步建议"（P23 实时通知中心）。
>
> **保留原因：** 作为设计探索性文档，保留供未来参考；若业务优先级变化，可基于此文档重启。
